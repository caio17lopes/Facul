'''
O que é Json
    É um formato de daqdso leve e de facil leitura e escrita
    Utilizado para representar estruturas de dados simples/complexas em formato de texto
    Amplamente utilizado para troca de daos entre um cliente e um servidor, sendo uma forma popular de comunicação em APIs

Como usar JSON no Python
    Biblioteca Json(já incluida no python)
    Fornece funções para trabalhar com Json
    Permite codificar (serializar) objetos 
    Python em JSON e decodificar
    (desserializar) JASON em objetos Python
'''
